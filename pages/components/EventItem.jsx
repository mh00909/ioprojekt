...
const EventItem = ({ event }) => {
  ...
  const hasDiscount = () => {
    if (weather && isToday(event.date)) {
      ...
      return (
        temperature < event.minTemperature || temperature > event.maxTemperature
      );
    }
    return false;
  };
  ...
  const handleCancelEvent = async () => {
    try {
      ...
      const response = await fetch(
        `${apiBaseUrl}/api/user/myevents/cancell/${event.id}`,
        {
          method: "POST",
          headers: {
  ...
  return (
    <div className="event-item">
      ...
      <p>
        Lokalizacja:{" "}
        <img
          src="https://c.animaapp.com/t0STnSju/img/map-2@2x.png"
          alt="Map Icon"
        />{" "}
        {event.location}
      </p>
      ...
      {weather && (
        <div className="weather-forecast-content">
          <p>
            Temperatura z API:{" "}
            {weather.main && Math.round(weather.main.temp - 273.15)}°C
          </p>
        </div>
      )}
